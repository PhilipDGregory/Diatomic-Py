...
def transition_plot(energies,states,gs,Nmax,I1,I2,TDMs=None,
    ...
    for j,d in enumerate(dz):
        ...
        if j<max(l2) and f[j]<maxf:
            if abs(d)>0 and j<max(l2):
                ...
                if print_vals == True:
                    print('sigma_minus: ' + str(f[j]+Offset) + 'MHz , TDM:' + str(numpy.abs(d)))
            ...
            if abs(d)>0 and j<max(l2):
                ...
                if print_vals == True:
                    print('sigma_plus: ' + str(f[j]+Offset) + 'MHz , TDM:' + str(numpy.abs(d)))
            ...
            if abs(d)>0 and j<max(l2):
                ...
                if print_vals == True:
                    print('pi: ' + str(f[j]+Offset) + 'MHz , TDM:' + str(numpy.abs(d)))
    ...
    if Offset != 0:
        ax[0].set_ylabel("Energy/$h$ (MHz) - {:.3f} MHz".format(Offset+minf-20*1e3*prefactor))
    else:
